...
//  printf("Break point 6\n");
    ...
    struct all_devices_interrupts all_received_devices_interrupts = {false, false, false, false, false, false, false,
                                                                     false};
    ...
    struct game_images game_loaded_images = {&game, &character, &monster, &game_over, &character_2_pontos, &number_0,
                                             &number_1, &number_2, &number_3, &number_4, &number_5, &number_6,
                                             &number_7, &number_8, &number_9};
    struct high_score_images high_score_loaded_images = {&character_2_pontos, &character_tracinho, &number_0, &number_1,
        ...
        if (is_start_of_screen) {
            ...
            switch (current_game_state) {
                ...
                case GAME:
                    ...
                    load_game_state_to_game_buffer(&all_game_entities_position, &current_game_values,
                                                   &game_loaded_images, game_frame_buffer);
                    break;
                case GAME_OVER:
...
//                    printf("Inside keyboard interrupt\n");
                        if (util_sys_inb(KBC_STATUS_REG, &status) !=
                            0) { // We test the function that reads the status from the status register, to check if we didn't have a communication error
                            return 1;
                        }
                        ...
                        if (scancode ==
                            FIRST_BYTE) { // Let's evaluate if the first byte of the scancode is 0xE0 (which would mean that the scancode has 2 bytes to be read)
                        ...
                        } else if (full_scancode[0] ==
                                   FIRST_BYTE) { // This condition is only true in the interruption immediatly after the scancode is 0xE0
...
//    printf("Finished interrupts\n");
        ...
        if (!all_received_devices_interrupts.is_timer_second_interrupt &&
            !all_received_devices_interrupts.is_timer_tick_interrupt && !all_received_devices_interrupts.W &&
            !all_received_devices_interrupts.A && !all_received_devices_interrupts.S &&
            !all_received_devices_interrupts.D && !all_received_devices_interrupts.is_mouse_move_interrupt &&
            !all_received_devices_interrupts.m1) {
            continue;
        }
//    printf("Interrupt to be handled\n");
        ...
        switch (current_game_state) {
            case MAIN_MENU:
                ...
                handle_main_menu(mouse_position_x, mouse_position_y, &current_game_state, &is_start_of_screen,
                                 &close_application);
                break;
            case GAME:
                ...
                if (all_received_devices_interrupts.is_timer_second_interrupt) {
                    handle_game_timer_second_interrupt(&all_game_entities_position, &current_game_values,
                                                       &last_key_pressed,
            ...
            case GAME_OVER:
                ...
                handle_game_over_interrupt(mouse_position_x, mouse_position_y, &current_game_state,
                                           &is_start_of_screen);
                break;
            case HIGH_SCORE:
                if (!all_received_devices_interrupts.m1) {
                ...
                handle_high_score_interrupt(mouse_position_x, mouse_position_y, &current_game_state,
                                            &is_start_of_screen);
                break;
        }
        ...
        if (!was_game_frame_buffer_changed && mouse_position_x == previous_mouse_position_x &&
            mouse_position_y == previous_mouse_position_y) {
            continue;
        }
        memcpy(mouse_frame_buffer, game_frame_buffer, length_frame_buffer);
        image_load_to_frame_buffer(&cursor, mouse_position_x - 16, mouse_position_y - 16, mouse_frame_buffer);
