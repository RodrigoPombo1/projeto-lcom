...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
    ...
    timer_get_conf(timer,
                   &controlWord); // We extract the control word of the timer "timer" and pass it by reference to the previous variable
    controlWord = ((BIT(3) | BIT(2) | BIT(1) | BIT(0)) & controlWord) |
                  TIMER_LSB_MSB; // We establish a new control word, where we assign the original LSB and activate the '11' initialization mode

    if (sys_outb(TIMER_CTRL, controlWord) !=
        0) { // We test the function that sends the new controlWord to the timer control register
        return 1;
    }
...
int (timer_display_conf)(uint8_t timer, uint8_t st, enum timer_status_field field) {
    ...
    if (timer_print_config(timer, field, timer_data) !=
        0) { // We test the function that prints the requested configuration info
        return 1;
    }
    return 0;
}
